...
public $template_page_1;
public $title;
public $max_page_width;
public $header_data;
public $column_widths;
public $header_aligns;
public $data_aligns;
public $column_width_total;
public $default_left_margin;
public $title_y_position;

public function set_fpdi_template() {
	if(isset($this->template_page_1)) {
		$this->useTemplate($this->template_page_1);

		$current_font_family = $this->FontFamily;
		$current_font_style = $this->FontStyle;
		$current_font_size = $this->FontSizePt;
		
		if(!isset($this->default_left_margin)) {
			$this->default_left_margin = $this->lMargin;
		}
		$this->SetLeftMargin($this->default_left_margin);
		$this->SetFont('Courier', 'B', 14);
		$this->SetY($this->title_y_position);
		$this->MultiCell($this->max_page_width, 10, $this->title, 0, 'C');
		$this->Ln(2);
		
		$this->SetFont('Courier', 'B', 10);
		$this->SetWidths($this->column_widths);
		$this->SetAligns($this->header_aligns);
		
		$left_margin = (($this->max_page_width - $this->column_width_total) / 2 ) + $this->lMargin;
		
		$this->SetLeftMargin($left_margin);
		$this->RowWithBorder($this->header_data);
		
		$this->SetFont($current_font_family, $current_font_style, $current_font_size);
		$this->SetAligns($this->data_aligns);
	}
